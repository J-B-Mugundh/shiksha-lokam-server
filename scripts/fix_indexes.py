...
class TestLFACreation:
    ...
    def test_create_lfa(self, api_base_url):
        ...
        if response.status_code not in [200, 201]:
            ...
            if "missing" in error_msg.lower() or "field required" in error_msg.lower():
                print("\n   📝 Required fields that might be missing:")
                print("      - name: LFA name")
                print("      - challenge_statement: Challenge statement")
                print("      - outcomes: Array of outcome objects")
                print("      - impacts: Array of impact objects")
